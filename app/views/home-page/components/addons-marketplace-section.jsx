"use client";

import { useState } from "react";
import Link from "next/link";
import Image from "next/image";
import { ArrowUpRight, ArrowRight } from "lucide-react";
import SectionHeading from "../../layouts/section-heading";
import PageWrapper from "@/app/components/page-wrapper";
import { getFeaturedAddons } from "@/app/data/addons";

export default function AddonsMarketplaceSection() {
  const addons = getFeaturedAddons();

  // Desktop preview
  const [activeId, setActiveId] = useState(addons[0]?.id);

  // Mobile / tablet accordion
  const [mobileOpenId, setMobileOpenId] = useState(addons[0]?.id);

  const activeItem =
    addons.find((item) => item.id === activeId) ?? addons[0];

  return (
    <PageWrapper className="bg-[#FBFAF7]">
      <style>{`
        @keyframes addon-image-in {
          from {
            opacity: 0;
            transform: scale(0.985);
          }
          to {
            opacity: 1;
            transform: scale(1);
          }
        }

        .addon-image-in {
          animation: addon-image-in 0.55s cubic-bezier(0.22, 1, 0.36, 1);
        }

        @media (prefers-reduced-motion: reduce) {
          .addon-image-in {
            animation: none;
          }
        }
      `}</style>

      <div>
        <SectionHeading
          eyebrow={"APP MARKETPLAC"}
          title={"Add-ons that supercharge your restaurant POS."}
          highlight={"supercharge"}
        />

        {/* =========================================================
            DESKTOP: hover rail + single image
        ========================================================= */}
        <div className="hidden lg:grid lg:grid-cols-2 lg:items-center lg:gap-14">
          <ul role="list" className="flex flex-col gap-4">
            {addons.map((item) => {
              const isActive = item.id === activeItem.id;

              return (
                <li
                  key={item.id}
                  onMouseEnter={() => setActiveId(item.id)}
                  className={`border-l-2 pl-4 transition-colors duration-500 ease-out ${
                    isActive
                      ? "border-l-primary-500"
                      : "border-l-[#E4E0D6]"
                  }`}
                >
                  <Link
                    href={`/addons/${item.id}`}
                    className="group block rounded-sm py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C6432B] focus-visible:ring-offset-2 focus-visible:ring-offset-[#FBFAF7]"
                  >
                    {/* Title */}
                    <span
                      className={`block tracking-tight transition-all duration-500 ease-out ${
                        isActive
                          ? "text-[20px] font-semibold text-[#12141A]"
                          : "text-[19px] font-medium text-[#8B8677] group-hover:text-[#12141A]"
                      }`}
                    >
                      {item.title}
                    </span>

                    {/* Description */}
                    <div
                      className="grid overflow-hidden transition-[grid-template-rows] duration-650 ease-[cubic-bezier(0.22,1,0.36,1)]"
                      style={{
                        gridTemplateRows: isActive ? "1fr" : "0fr",
                      }}
                    >
                      <div className="min-h-0 overflow-hidden">
                        <div
                          className={`transform transition-all duration-500 ease-out ${
                            isActive
                              ? "translate-y-0 opacity-100"
                              : "-translate-y-2 opacity-0"
                          }`}
                        >
                          <span className="mt-1.5 block pt-0.5 text-[13.5px] leading-relaxed text-[#6B6759]">
                            {item.description}
                          </span>

                          <span className="mt-1.5 inline-flex items-center gap-1 text-[12.5px] font-medium text-primary-500">
                            Explore
                            <ArrowUpRight
                              aria-hidden="true"
                              className="h-3 w-3"
                            />
                          </span>
                        </div>
                      </div>
                    </div>
                  </Link>
                </li>
              );
            })}
          </ul>

          {/* Single product image */}
          <div
            key={activeItem.id}
            className="addon-image-in relative aspect-[4/3] w-full overflow-hidden rounded-2xl border border-[#E4E0D6] bg-white shadow-[0_16px_36px_-22px_rgba(18,20,26,0.28)]"
          >
            <Image
              src={activeItem.image}
              alt={activeItem.title}
              fill
              sizes="45vw"
              priority
              className="object-cover object-top"
            />
          </div>
        </div>

        {/* =========================================================
            MOBILE / TABLET: accordion
        ========================================================= */}
        <div className="lg:hidden">
          <ul role="list" className="flex flex-col gap-6">
            {addons.map((item) => {
              const isActive = item.id === mobileOpenId;

              return (
                <li
                  key={item.id}
                  className={`border-l-2 transition-colors duration-500 ease-out ${
                    isActive
                      ? "border-l-primary-500"
                      : "border-l-[#E4E0D6]"
                  }`}
                >
                  {/* Accordion button */}
                  <button
                    type="button"
                    onClick={() =>
                      setMobileOpenId(isActive ? null : item.id)
                    }
                    aria-expanded={isActive}
                    className="block w-full rounded-sm pl-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C6432B] focus-visible:ring-offset-2 focus-visible:ring-offset-[#FBFAF7]"
                  >
                    <span
                      className={`block text-[18px] tracking-tight transition-all duration-500 ease-out ${
                        isActive
                          ? "font-semibold text-[#12141A]"
                          : "font-medium text-[#8B8677]"
                      }`}
                    >
                      {item.title}
                    </span>
                  </button>

                  {/* Smooth accordion */}
                  <div
                    className="grid overflow-hidden transition-[grid-template-rows] duration-650 ease-[cubic-bezier(0.22,1,0.36,1)]"
                    style={{
                      gridTemplateRows: isActive ? "1fr" : "0fr",
                    }}
                  >
                    <div className="min-h-0 overflow-hidden">
                      <div
                        className={`pl-4 pt-4 pb-1 transform transition-all duration-500 ease-out ${
                          isActive
                            ? "translate-y-0 opacity-100"
                            : "-translate-y-3 opacity-0"
                        }`}
                      >
                        {/* Product image */}
                        <div className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-[#E4E0D6] bg-white shadow-[0_10px_24px_-16px_rgba(18,20,26,0.24)]">
                          <Image
                            src={item.image}
                            alt={item.title}
                            fill
                            sizes="100vw"
                            className="object-cover object-top"
                          />
                        </div>

                        {/* Description */}
                        <p className="mt-4 text-[13.5px] leading-relaxed text-[#6B6759]">
                          {item.description}
                        </p>

                        {/* Explore */}
                        <Link
                          href={`/addons/${item.id}`}
                          className="mt-2.5 inline-flex items-center gap-1 rounded-sm text-[13px] font-medium text-primary-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C6432B]"
                        >
                          Explore
                          <ArrowUpRight
                            aria-hidden="true"
                            className="h-3 w-3"
                          />
                        </Link>
                      </div>
                    </div>
                  </div>
                </li>
              );
            })}
          </ul>
        </div>

        {/* View all */}
        <div className="mt-10 flex justify-center lg:mt-12">
          <Link href="/addons" className="btn btn-primary">
            View All Addons
            <ArrowRight
              aria-hidden="true"
              className="h-3.5 w-3.5"
            />
          </Link>
        </div>
      </div>
    </PageWrapper>
  );
}
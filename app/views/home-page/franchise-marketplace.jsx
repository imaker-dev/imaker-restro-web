...
export default function FranchiseMarketplace() {
  ...
  return (
    <PageWrapper>
      <SectionHeader
        badge={"Verified brands powered by our POS platform"}
        title={`Own a piece of India's fastest-growing food brands`}
        description={`Curated franchise opportunities from businesses already running on our platform — with verified revenue, real operating data, and a guided path from inquiry to opening day.`}
      />
      ...
      {filters?.stats && (
        <div className="mb-5 text-[13px] text-slate-500">
          ...
          {filters.stats.min_investment && (
            <>
              {" "}
              • Investment range: {formatINR(filters.stats.min_investment)} –{" "}
              {formatINR(filters.stats.max_investment)}
            </>
          )}
        </div>
      )}
